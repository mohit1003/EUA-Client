...
public class MQConsumerServiceTest {
    ...
    public void setupUp() throws JsonProcessingException {
        ...
        mqConsumerService = Mockito.mock(MQConsumerService.class);
...
//        Mockito.verify(mqConsumerService).getAckResponseResponseEntity(requestBody, null, 1 );
    ...
    void sendNackResponse() {
        mqConsumerService.sendNackResponse(anyString(),anyString());
        Mockito.verify(mqConsumerService).sendNackResponse(anyString(),anyString());

    }

    @Test
    void euaToGatewayConsumer() throws IOException {
        mqConsumerService.euaToGatewayConsumer(any(), any(),anyLong());
        Mockito.verify(mqConsumerService).euaToGatewayConsumer(any(), any(),anyLong());
    }

    @Test
    void gatewayToEuaConsumer() throws IOException {
        mqConsumerService.gatewayToEuaConsumer(any());
        Mockito.verify(mqConsumerService).gatewayToEuaConsumer(any());
    }
}